...
async function genericGoogleSearchArticleSnippet(params: { query: string; articleNumber: number }): Promise<RetrievedLegalSnippet | null> {
  const q = String(params.query ?? "").trim();
  if (!q) return null;
  if (!/(نص\s*المادة|تنص\s*المادة|المادة\s*\d+)/.test(q)) return null;
  ...
  // Try DuckDuckGo with Google bang first
  const ddgWithGoogleUrl = `https://duckduckgo.com/html/?q=${encodeURIComponent(`!g ${expandedQuery}`)}`;
  try {
    const res = await httpGetText(ddgWithGoogleUrl, {
      accept: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
      "user-agent": ENV.legalCrawlerUserAgent,
    });
    if (res.ok) {
  ...
  // Fallback: lightweight Google scraping (no API)
  const googleSearchUrl = `https://www.google.com/search?q=${encodeURIComponent(expandedQuery)}`;
  try {
    const res = await httpGetText(googleSearchUrl, {
      accept: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
      "user-agent": ENV.legalCrawlerUserAgent,
    });
    if (!res.ok) return null;
    const html = await res.text();
    // Extract first result link (very lightweight)
    ...
    if (!isAllowedWebFallbackHost(host)) return null;
    const pageRes = await httpGetText(href, {
      accept: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
      "user-agent": ENV.legalCrawlerUserAgent,
    });
...
export async function retrieveLegalSnippets(params: {
  ...
  if (n !== null) {
    const web = await genericGoogleSearchArticleSnippet({ query: params.query, articleNumber: n });
    if (web) return [web];
  }
